...
(function () {
  ...
  function renderCard(p) {
    ...
    return '<article class="product-card" data-product-id="' + p.id + '"><div class="product-card__media">' + badge(p) +
      ...
      '<a class="btn btn-outline btn-sm" href="' + ROOT + 'pages/product.html?id=' + p.id + '">View Details</a></div></div></article>';
  }
  ...
  on(d, 'click', function (e) {
    ...
    $('[data-quick-view-body]', modal).innerHTML = '<div class="quick-view">' +
      ...
      '<div class="pdp__buy" style="display:flex;gap:var(--sp-3);flex-wrap:wrap">' +
      '<a class="btn btn-primary" href="' + ROOT + 'pages/contact.html?type=order&product=' + encodeURIComponent(p.name) + '">Enquire Now</a>' +
      '<a class="btn btn-outline" href="' + ROOT + 'pages/product.html?id=' + p.id + '">View Details ' + svg('arrow-right') + '</a></div></div></div>';
    openOverlay(modal, b);
  });
  ...
  if (shopGrid) (function () {
    ...
    function apply(keepShown) {
      ...
      if (priceInput && state.max < maxPrice) chips.push(chip('Under ' + inr(state.max), function () { state.max = maxPrice; priceInput.value = maxPrice; updatePriceOut(); }));
      if (state.rating) chips.push(chip(state.rating + '+ stars', function () { state.rating = 0; if ($$('[data-filter-rating]')[0]) $$('[data-filter-rating]')[0].checked = true; }));
      if (state.sale) chips.push(chip('On sale', function () { state.sale = false; var s = $('[data-filter-sale]'); if (s) s.checked = false; }));
      if (state.isNew) chips.push(chip('New arrivals', function () { state.isNew = false; var n = $('[data-filter-new]'); if (n) n.checked = false; }));
    ...
    if (priceInput) on(priceInput, 'input', function () { state.max = parseInt(priceInput.value, 10); updatePriceOut(); debouncedApply(); });
    ...
    $$('[data-filters-reset]').forEach(function (b) {
      on(b, 'click', function () {
        ...
        if ($$('[data-filter-rating]')[0]) $$('[data-filter-rating]')[0].checked = true;
  ...
  if (pdp) (function () {
    ...
    if (addBtn) addBtn.setAttribute('data-add-to-cart', p.id);
    var enqBtn = $('[data-pdp-enquire]');
    if (enqBtn) enqBtn.href = 'contact.html?type=order&product=' + encodeURIComponent(p.name);
          ...
          return '<div class="routine-step">' +
            ...
            '<div class="routine-step__item">' +
              ...
              '<div class="cluster" style="gap:4px">' +
                ...
                '<a class="btn btn-outline btn-sm" href="pages/product.html?id=' + (p.id || s.product) + '">View Details</a>' +
              '</div>' +
            '</div>' +
          '</div>';
        }).join('');
