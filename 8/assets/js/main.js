...
(() => {
  ...
  const navItems = [
    ["products", "Shop", "pages/products.html"],
        ...
        <header class="site-header" data-header>
          ...
          <nav class="nav-shell" aria-label="Primary navigation">
            <a class="brand" href="${path("index.html")}" aria-label="Meeple and Marrow home">
              <span class="brand__mark" aria-hidden="true"><img src="${path("assets/images/brand/brand-mark.svg")}" alt="" width="40" height="40"></span>
  ...
  // Color Theme Toggle (All buttons on page)
  ...
  // Text Direction (LTR / RTL) Toggle (All buttons on page)
  const updateDirectionButtons = () => {
    const isRtl = html.dir === "rtl";
    doc.querySelectorAll("[data-direction-toggle]").forEach((button) => {
  ...
  // Delegated click handler for theme and direction toggles
  doc.addEventListener("click", (e) => {
    ...
    const dirBtn = e.target.closest("[data-direction-toggle]");
    if (dirBtn) {
  ...
  recommender?.addEventListener("submit", (event) => {
    ...
    const gameDatabase = {
      solo: {
        thoughtful: { title: "Verdant Paths", cat: "strategy", price: 3599, players: "1–4 players", time: "55 min", desc: "A meditative landscape puzzle with a rich solo campaign and lush wooden pieces." },
      ...
      target.innerHTML = `
        <div class="rec-card">
          ...
          <div class="rec-card__actions">
            ...
            <button class="btn btn--small" type="button" data-add-game="${match.title}" data-game-price="${match.price}" data-game-cat="${match.cat}" data-game-meta="${match.players} · ${match.time}">
              <span>Add to bag</span>
              ${icon("bag")}
            </button>
          </div>
  ...
  const categoryDetailsData = {
    strategy: {
      ...
      featured: [
        { name: "Verdant Paths", price: "₹3,599" },
  ...
  // Forgot Password Dialog handling
  doc.addEventListener("click", (e) => {
    const forgotBtn = e.target.closest("[data-forgot-open]");
    if (forgotBtn) {
      const d = doc.querySelector("[data-forgot-dialog]");
      if (d) {
        d.showModal();
    ...
    const closeBtn = e.target.closest("[data-forgot-close]");
    if (closeBtn) {
      doc.querySelector("[data-forgot-dialog]")?.close();
      return;
    }
  });

  doc.querySelector("[data-forgot-dialog]")?.addEventListener("click", (e) => {
    const d = doc.querySelector("[data-forgot-dialog]");
    if (e.target === d) d?.close();
  });
  ...
  const searchIndex = [
    ...
    { title: "Verdant Paths", type: "Game · Strategy", terms: "verdant paths trails landscape nature solo campaign offer 1-4 players ₹3599", url: "pages/products.html#strategy" },
